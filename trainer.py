...
def evaluate(ds, sampler, model, batch_size=50):
    """
    Evaluate with accuracy
    """
    model.eval()
    val_loader = DataLoaderX(ds, batch_size, num_workers=4, pin_memory=True, sampler=sampler)
    correct = 0
    predicts = []
    labels = []
    with torch.no_grad():
...
def baseline(ds, y_labels_num, backend, batch_size=50, epochs=20, start_lr=0.001,
    ...
    for epoch in range(starting_epoch, starting_epoch + epochs):
        train_sampler, eval_sampler, class_weights = split_train_validate(ds, y_labels_num)
        train_loader = DataLoaderX(ds, batch_size, shuffle=False, pin_memory=True,
                                   num_workers=4, sampler=train_sampler)
        criterion = nn.CrossEntropyLoss(weight=class_weights, label_smoothing=0.)
        accuracy, f1_score = evaluate(ds, eval_sampler, net, batch_size)
...
def test(model, batch_size=50, softVote=False, TTA=True):
...
if __name__ == "__main__":
    ...
    if args.test_only:
        ds, y_labels_num = None, None
    else:
        full_data, full_label = data_prepare()
        ds, y_labels_num = dataset_generation(full_data, full_label)
    ...
    if checkpoints:
        ...
        checkpoint = None
    net = baseline(ds, y_labels_num, args.backbone, args.batch_size, args.epoch,
    ...
    test(net, args.batch_size, args.softvote, args.tta)
